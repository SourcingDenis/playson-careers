    ...
    <div className="pb-24 relative">
      ...
      <div className={`bg-zinc-900/50 border-b pt-12 pb-16 transition-colors duration-500 ${isHot ? 'border-playson-red/30' : 'border-zinc-800'}`}>
        <div className="mx-auto max-w-4xl px-4 md:px-6">
          <Link to={isEngineeringRole(job.title, job.department) ? "/engineering" : "/"} className="inline-flex items-center gap-2 text-sm text-zinc-400 hover:text-zinc-200 transition-colors mb-8">
            <ArrowLeft className="w-4 h-4" /> {isEngineeringRole(job.title, job.department) ? "Back to engineering" : "Back to careers"}
          </Link>
